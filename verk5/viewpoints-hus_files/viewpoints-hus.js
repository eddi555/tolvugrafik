...
window.onload = function init()
{
    ...
    window.addEventListener("keydown", function(e){
        switch( e.keyCode ) {
            ...
            case 57:
                view = 9;
                document.getElementById("Viewpoint").innerHTML = "9: Horfa alltaf á bílinn ofan af húsi";
                break;
            case 48:
                view = 0;
                document.getElementById("Viewpoint").innerHTML = "0: Til hliðar við bílinn";
                break;
            case 38:    // up arrow
                height += 2.0;
                document.getElementById("Height").innerHTML = "Viðbótarhæð: "+ height;
...
function render()
{
	...
	case 8:
	    ...
	    break;
	case 9:
        mv = lookAt( vec3(-40.0, 140.0, 20.0+height), vec3(carXPos, carYPos, 0.0), vec3(0.0, 0.0, 1.0 ) );
	    drawScenery( mv );
	    mv = mult( mv, translate(carXPos, carYPos, 0.0) );
	    mv = mult( mv, rotateZ( carDirection ) ) ;
	    drawCar( mv );
        break;
    case 0:
	    mv = lookAt( vec3(2.0, 20.0, 10.0), vec3(0.0+spinY, 0.0, 2.0), vec3(0.0, 0.0, 1.0 ) );
        drawScenery( mv );
        mv = mult( mv, translate(carXPos, carYPos, 0.0) );
	    mv = mult( mv, rotateZ( carDirection ) ) ;
	    drawCar( mv );
